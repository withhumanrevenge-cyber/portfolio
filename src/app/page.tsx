...
import { ArrowRight, Play, LayoutGrid, Camera, Film } from "lucide-react";
      ...
      <main className="min-h-screen px-6 md:px-20 pt-32 pb-20 relative overflow-hidden">
        <div className="fixed top-1/2 left-0 w-full -translate-y-1/2 pointer-events-none select-none z-[-1] overflow-hidden">
          <motion.h2
            ...
            className="text-[30vw] font-outfit font-black text-white/[0.02] dark:text-white/[0.01] whitespace-nowrap leading-none"
          >
        ...
        <section className="max-w-7xl mx-auto mb-40">
          <div className="flex flex-col gap-8">
            ...
            >
              <a href="#work" className="px-8 py-4 bg-accent text-white rounded-full font-bold flex items-center gap-2 hover:scale-105 transition-transform interactive">
                See My Work <ArrowRight size={18} />
              </a>
              <a href="mailto:monuraj88703@gmail.com" className="px-8 py-4 glass-card border-glass-border rounded-full font-bold flex items-center gap-2 hover:bg-white/10 transition-colors interactive">
          ...
          <div className="flex justify-between items-end mb-12">
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`w-12 h-12 glass-card flex items-center justify-center rounded-full interactive transition-all ${viewMode === "list" ? "bg-accent text-white scale-110" : "hover:text-accent"}`}
              >
                <Film size={20} />
              </button>
              <button
                onClick={() => setViewMode("grid")}
                className={`w-12 h-12 glass-card flex items-center justify-center rounded-full interactive transition-all ${viewMode === "grid" ? "bg-accent text-white scale-110" : "hover:text-accent"}`}
              >
                <LayoutGrid size={20} />
              </button>
            </div>
              ...
              <a key={project.id} href={project.href} target="_blank" rel="noopener noreferrer" className="block">
                <TiltCard>
                  <motion.div
                    ...
                    viewport={{ once: true }}
                    className={`group relative overflow-hidden rounded-[30px] glass-card transition-all duration-500 ${viewMode === "grid" ? "h-[500px]" : "h-[250px] flex gap-8 items-center"}`}
                  >
                    ...
                    <div className={viewMode === "list" ? "w-1/3 h-full overflow-hidden relative" : "absolute inset-0 w-full h-full"}>
                      <Image
                        src={project.image}
                        alt={project.title}
                        fill
                    ...
                    <div className={`${viewMode === "grid" ? "absolute inset-0 pointer-events-none" : "relative flex-1 pr-12"}`}>
                      {viewMode === "grid" && <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-60" />}

                      <div className={`${viewMode === "grid" ? "absolute bottom-0 left-0 w-full p-8" : ""} flex justify-between items-end`}>
                        <div>
                          <span className="text-xs uppercase tracking-widest text-accent font-bold mb-2 block">{project.category}</span>
                          <h4 className={`${viewMode === "grid" ? "text-3xl" : "text-5xl"} font-outfit font-bold`}>{project.title}</h4>
                        </div>
                        <div className="w-14 h-14 bg-white/10 backdrop-blur-md rounded-full flex items-center justify-center border border-white/20 -translate-y-4 opacity-0 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500 interactive">
        ...
        <div className="fixed bottom-10 right-10 flex flex-col gap-3 glass-card p-6 border-white/5 backdrop-blur-md z-50">
          <div className="flex items-center gap-2 mb-2">
            <div className="w-2 h-2 rounded-full bg-accent animate-pulse shadow-[0_0_10px_var(--accent)]" />
            <div className="text-[10px] tracking-[0.2em] uppercase font-black text-white/50">Core Technologies</div>
          </div>
          {[95, 90, 85].map((width, i) => (
            <div key={i} className="flex items-center gap-3">
              ...
              <span className="text-[8px] font-mono text-accent/60">0{i + 1}_{i === 0 ? "REACT" : i === 1 ? "NEXT.JS" : "TYPESCRIPT"}</span>
            </div>
          ))}
        </div>
      </main>
